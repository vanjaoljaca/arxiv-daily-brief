...
export async function POST(request: Request) {
  ...
  const contentType = request.headers.get("x-content-type") === "hn" ? "hn" : "arxiv";
  ...
  const key = `voice/${contentType}/${deliveryDate}/${editionVersion}/${id}/000000`;
  const { DB, AUDIO } = bindings();
  await AUDIO.put(key, bytes, { httpMetadata: { contentType: request.headers.get("content-type") || "audio/wav" } });
  await DB.batch([
    DB.prepare(`INSERT INTO voice_sessions (id, owner_email, content_type, delivery_date, edition_version, mime_type, started_at, finished_at, duration_ms, total_chunks, total_bytes, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, 'waiting')`)
      .bind(id, "deployment-verifier@local", contentType, deliveryDate, editionVersion, request.headers.get("content-type") || "audio/wav", now, now, 100, bytes.byteLength),
  ...
  return Response.json({ id, contentType, status: "waiting" }, { status: 201 });
}
